...
def _load_human_use_lumped_table(paths: DevpUSPaths, basin_id: str) -> pd.Series | None:
    path = paths.human_use_dir / "lumped_camels_us.csv"
    if not path.exists():
        return None
    df = pd.read_csv(path)
...
def _load_human_use_node_table(paths: DevpUSPaths, huc_str: str, basin_id: str) -> pd.DataFrame | None:
    path = paths.human_use_dir / "distributed" / f"{huc_str}_{basin_id}.csv"
...
def _normalize_huc_codes(values: list[str] | tuple[str, ...] | set[str] | None) -> set[str] | None:
    if not values:
        return None
    return {str(value).zfill(2) for value in values}


def _record_path(root: Path, regime: str, huc_str: str, basin_id: str) -> Path:
    return root / regime / f"{huc_str}_{basin_id}.pt"


def _maybe_swi_payload(ds: xr.Dataset) -> torch.Tensor | None:
    if "dynamic_GloFAS_soil_wetness_index" not in ds.data_vars:
        return None
    return torch.from_numpy(_to_numpy_array(ds["dynamic_GloFAS_soil_wetness_index"]))


def build_lumped_tensors(paths: DevpUSPaths, feature_registry: FeatureRegistry, registry: pd.DataFrame) -> list[str]:
    output_dir = paths.tensor_dir / "lumped"
    output_dir.mkdir(parents=True, exist_ok=True)
    written: list[str] = []
    for _, row in registry.iterrows():
        if not row["has_lumped_inventory"]:
            continue
        basin_id = str(row["gauge_id"]).zfill(8)
        huc_str = str(row["huc_02_str"]).zfill(2)
        ds = _analysis_slice(xr.open_zarr(paths.source_us_root / "lumped_inventory" / huc_str / f"{basin_id}.zarr"))
        ...
        swi = _maybe_swi_payload(ds)
        if swi is not None:
            payload["swi"] = swi
        output_path = _record_path(paths.tensor_dir, "lumped", huc_str, basin_id)
        torch.save(payload, output_path)
        written.append(str(output_path))
    return written


def build_distributed_tensors(paths: DevpUSPaths, feature_registry: FeatureRegistry, registry: pd.DataFrame) -> list[str]:
    ...
    for _, row in registry.iterrows():
        ...
        basin_id = str(row["gauge_id"]).zfill(8)
        huc_str = str(row["huc_02_str"]).zfill(2)
        ds = _analysis_slice(xr.open_zarr(paths.source_us_root / "inventory" / huc_str / f"{basin_id}.zarr"))
        static_names, static_values = _distributed_static_features(ds)
        human = _load_human_use_node_table(paths, huc_str, basin_id)
        if human is not None:
            feature_names = [feature for feature in HUMAN_USE_FEATURES if feature in human.columns]
            human_values = np.nan_to_num(
                human[feature_names].to_numpy(dtype=np.float32),
                nan=0.0,
                posinf=0.0,
                neginf=0.0,
            )
            static_values = np.concatenate([static_values, human_values], axis=1)
            static_names = static_names + feature_names
        dynamic_names, dynamic = _distributed_dynamic_features(ds, feature_registry)
        edge_index, edge_weight = _edge_index_from_csv(paths.source_us_root / "graph_files" / huc_str / basin_id / "edges.csv")
        payload = {
            "basin_id": basin_id,
            "huc_02_str": huc_str,
            "regime": "distributed",
        ...
        if swi is not None:
            payload["swi"] = swi
        output_path = _record_path(paths.tensor_dir, "distributed", huc_str, basin_id)
        torch.save(payload, output_path)
        written.append(str(output_path))
    return written


def build_semidistributed_tensors(paths: DevpUSPaths, registry: pd.DataFrame) -> list[str]:
    ...
    for _, row in registry.iterrows():
        basin_id = str(row["gauge_id"]).zfill(8)
        huc_str = str(row["huc_02_str"]).zfill(2)
        record_path = paths.tensor_dir / "distributed" / f"{huc_str}_{basin_id}.pt"
        if not record_path.exists():
            continue
        record = torch.load(record_path, map_location="cpu", weights_only=False)
        assigned_path = paths.source_us_root / "nested_gauges" / "nesting_info" / huc_str / basin_id / "nodes_coords_assigned.csv"
        edge_gauge_path = paths.source_us_root / "nested_gauges" / "nesting_info" / huc_str / basin_id / "edges_gauges.csv"
        if assigned_path.exists():
            ...
            node_count = int(record["static"].shape[0])
        ...
        swi_rows = []
        dynamic_rows = []
        outlet_index_map = {basin_id: int(record["outlet_idx"])}
        nested_json = paths.source_us_root / "nested_gauges" / "nesting_info" / huc_str / basin_id / "nested_gauges.json"
        if nested_json.exists():
            nested = json.loads(nested_json.read_text())
            outlet_index_map[str(nested["downstream_gauge"]).zfill(8)] = int(record["outlet_idx"])
        ...
        if swi_rows:
            payload["swi"] = torch.from_numpy(np.stack(swi_rows, axis=1).astype(np.float32))
        output_path = _record_path(paths.tensor_dir, "semidistributed", huc_str, basin_id)
        torch.save(payload, output_path)
        written.append(str(output_path))
    return written
...
def build_monthly_tensors(paths: DevpUSPaths, regime: str) -> list[Path]:
...
def _fit_standard_scalers(
    ...
    dyn_sum = np.zeros(len(dynamic_names), dtype=np.float64)
    dyn_sq_sum = np.zeros(len(dynamic_names), dtype=np.float64)
    stat_sum = np.zeros(len(static_names), dtype=np.float64)
    stat_sq_sum = np.zeros(len(static_names), dtype=np.float64)
    dyn_count = 0
    stat_count = 0
    for record_path in record_paths:
        ...
        dyn = record["dynamic"][warmup_mask].reshape(-1, len(dynamic_names)).numpy().astype(np.float64, copy=False)
        stat = record["static"].reshape(-1, len(static_names)).numpy().astype(np.float64, copy=False)
        dyn_sum += dyn.sum(axis=0)
        dyn_sq_sum += np.square(dyn).sum(axis=0)
        stat_sum += stat.sum(axis=0)
        stat_sq_sum += np.square(stat).sum(axis=0)
        dyn_count += int(dyn.shape[0])
        stat_count += int(stat.shape[0])
    if dyn_count == 0 or stat_count == 0:
        raise ValueError("Cannot fit scalers with zero dynamic or static samples.")
    dyn_mean = dyn_sum / dyn_count
    stat_mean = stat_sum / stat_count
    dyn_var = np.maximum(dyn_sq_sum / dyn_count - np.square(dyn_mean), 1e-12)
    stat_var = np.maximum(stat_sq_sum / stat_count - np.square(stat_mean), 1e-12)
    return {
        "dynamic_mean": torch.from_numpy(dyn_mean.astype(np.float32)),
        "dynamic_std": torch.from_numpy(np.sqrt(dyn_var).astype(np.float32)),
        "static_mean": torch.from_numpy(stat_mean.astype(np.float32)),
        "static_std": torch.from_numpy(np.sqrt(stat_var).astype(np.float32)),
    }
...
def fit_and_save_scalers(
    ...
    *,
    ...
    allowed_hucs: list[str] | None = None,
) -> dict[str, str]:
    written: dict[str, str] = {}
    allowed_huc_set = _normalize_huc_codes(allowed_hucs)
    for regime in regimes:
        regime_dir = paths.tensor_regime_dir(regime, frequency)
        record_paths = [path for path in sorted(regime_dir.glob("*.pt")) if path.name != "scalers.pt"]
        if allowed_huc_set is not None:
            record_paths = [path for path in record_paths if path.stem.split("_", 1)[0] in allowed_huc_set]
        if not record_paths:
            continue
        scalers = _fit_standard_scalers(
            record_paths,
            torch.load(record_paths[0], map_location="cpu", weights_only=False)["dynamic_feature_names"],
            torch.load(record_paths[0], map_location="cpu", weights_only=False)["static_feature_names"],
            split_spec=split_spec,
        )
        output_path = regime_dir / "scalers.pt"
        torch.save(scalers, output_path)
        written[regime] = str(output_path)
    return written
...
class LumpedWindowDataset(Dataset):
    def __init__(
        ...
        if allowed_huc_set is not None:
            records = [record for record in records if str(record.get("huc_02_str", "")).zfill(2) in allowed_huc_set]
...
def graph_batch_cache_dir(
    ...
    allowed_huc_set = _normalize_huc_codes(allowed_hucs)
    huc_key = "all" if not allowed_huc_set else "huc-" + "-".join(sorted(allowed_huc_set))
    cache_key = f"{dynamic_group}__{static_group}__h{history_length}__b{batch_size}__{huc_key}"
...
def build_graph_batch_cache(
    ...
    for split in ["train", "val", "test"]:
        dataset = dataset_cls(
            ...
            history_length,
            frequency=frequency,
            allowed_hucs=allowed_hucs,
        )
        out_dir = graph_batch_cache_dir(
            tensor_root,
            dynamic_group,
            static_group,
            history_length,
...
class GraphBatchDataset(Dataset):
    def __init__(
        ...
        self.batch_dir = graph_batch_cache_dir(
            tensor_root,
            dynamic_group,
            static_group,
            history_length,
