...
from .registry import build_us_registry
...
def _sample_static(path, lon: np.ndarray, lat: np.ndarray) -> np.ndarray:
...
def _sample_temporal_summary(path, lon: np.ndarray, lat: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ds = xr.open_dataset(path)
...
def _load_gdw_reservoir_counts(paths: DevpUSPaths, registry: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in registry.iterrows():
        ...
        huc_str = row["huc_02_str"]
        nodes = pd.read_csv(paths.source_us_root / "graph_files" / huc_str / basin_id / "nodes_coords.csv")
    ...
    bounds = (
        float(all_nodes["lon"].min() - 0.5),
        float(all_nodes["lat"].min() - 0.5),
        float(all_nodes["lon"].max() + 0.5),
        float(all_nodes["lat"].max() + 0.5),
    )
    gdw = gpd.read_file(reservoir_path, bbox=bounds)
    if gdw.empty:
        empty = pd.DataFrame(columns=["gauge_id", "gdw_reservoir_count"])
        return empty, empty
    geom = gdw.geometry
    if geom.geom_type.iloc[0] != "Point":
...
def extract_us_human_use(paths: DevpUSPaths) -> dict[str, str]:
    ...
    basin_reservoir_counts, semi_reservoir_counts = _load_gdw_reservoir_counts(paths, registry)
    basin_rows = []
    ...
    for _, row in registry.iterrows():
        ...
        ds = xr.open_zarr(paths.source_us_root / "inventory" / huc_str / f"{basin_id}.zarr")
        node_table["cellarea_km2"] = np.asarray(ds["static_GloFAS_cellarea_km2"].values, dtype=float)
        node_table.to_csv(distributed_dir / f"{huc_str}_{basin_id}.csv", index=False)

        assigned_path = paths.source_us_root / "nested_gauges" / "nesting_info" / huc_str / basin_id / "nodes_coords_assigned.csv"
        if assigned_path.exists():
            assigned = pd.read_csv(assigned_path)
            merged = node_table.merge(assigned[["lon", "lat", "assigned_gauge_id"]], on=["lon", "lat"], how="left")
        ...
        semi = _aggregate_weighted(merged, ["assigned_gauge_id"], "cellarea_km2").rename(columns={"assigned_gauge_id": "gauge_id"})
        ...
        semi.to_csv(semidistributed_dir / f"{huc_str}_{basin_id}.csv", index=False)
    ...
    lumped = pd.DataFrame(basin_rows).sort_values(["huc_02_str", "gauge_id"]).reset_index(drop=True)
    lumped_path = paths.human_use_dir / "lumped_camels_us.csv"
    lumped.to_csv(lumped_path, index=False)
    return {
        "distributed_dir": str(distributed_dir),
        "semidistributed_dir": str(semidistributed_dir),
        "lumped_table": str(lumped_path),
    }
