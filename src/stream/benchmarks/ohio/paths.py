...
def _find_upward(start: Path, parts: tuple[str, ...]) -> Path:
    for candidate in (start, *start.parents):
        path = candidate.joinpath(*parts)
        if path.exists():
            return path
    return Path("/data/projects/KDD2026").joinpath(*parts)
...
class DevpOhioPaths:
    ...
    def source_ohio_root(self) -> Path:
        return _find_upward(self.root, ("reference_materials", "zenodo", "03min_GloFAS_CAMELS-US_Ohio"))

    @property
    def source_lumped_reference_root(self) -> Path:
        return _find_upward(self.root, ("reference_materials", "zenodo", "lumped_surrogate_datasets"))
