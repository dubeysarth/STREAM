...
from devp_ohio.datasets import build_distributed_tensors, build_semidistributed_tensors, fit_and_save_scalers
...
def main() -> None:
    parser = build_parser("Build semi-distributed HUC05 tensor records.")
    args = parser.parse_args()
    paths = DevpOhioPaths.from_root(args.root)
    context = RunContext("build_semidistributed_tensors", paths, run_name=args.run_name, dry_run=args.dry_run)
    ...
    if not any((paths.tensor_dir / "distributed").glob("*.pt")):
        context.info("distributed tensors missing; staging them first for semidistributed aggregation")
        build_distributed_tensors(paths, feature_registry)
    written = build_semidistributed_tensors(paths, feature_registry)
    ...
    context.info("built %s semidistributed tensor records", len(written))
