...
from devp_ohio.metrics import MetricBundle
...
def _lumped_context(paths: DevpOhioPaths, run_id: str) -> pd.DataFrame:
    ...
    human = pd.read_csv(paths.human_use_dir / "lumped_huc05.csv")
    human["gauge_id"] = human["gauge_id"].astype(str).str.zfill(8)
    return perf.merge(human, on="gauge_id", how="left")


def _semi_context(paths: DevpOhioPaths, run_id: str) -> pd.DataFrame:
    predictions = pd.read_csv(paths.predictions_dir / f"{run_id}.csv")
    test = predictions[predictions["split"] == "test"].copy()
    test["basin_id"] = test["basin_id"].astype(str).str.zfill(8)
    ...
    perf = pd.DataFrame(rows)
    human_rows = []
    for path in sorted((paths.human_use_dir / "semidistributed").glob("05_*.csv")):
        df = pd.read_csv(path)
        basin_id = path.stem.replace("05_", "")
