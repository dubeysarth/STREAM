...
from devp_ohio.datasets import build_lumped_tensors, fit_and_save_scalers
...
def main() -> None:
    parser = build_parser("Build lumped HUC05 tensor records.")
    args = parser.parse_args()
    paths = DevpOhioPaths.from_root(args.root)
    context = RunContext("build_lumped_tensors", paths, run_name=args.run_name, dry_run=args.dry_run)
    ...
    written = build_lumped_tensors(paths, feature_registry)
    ...
    context.info("built %s lumped tensor records", len(written))
