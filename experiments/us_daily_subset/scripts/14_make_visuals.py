...
from devp_us.plots import PlotBundle
from devp_us.runtime import RunContext, build_parser


def _render_reference_graph(paths: DevpUSPaths) -> Path:
    graph_dirs = sorted(
        path
        for path in paths.source_us_root.glob("graph_files/*/*")
        if (path / "nodes_coords.csv").exists() and (path / "edges.csv").exists()
    )
    if not graph_dirs:
        raise FileNotFoundError("No reference graph directories found under CAMELS-US graph_files.")
    graph_dir = graph_dirs[0]
    huc_str = graph_dir.parent.name
    basin_id = graph_dir.name
    nodes = pd.read_csv(graph_dir / "nodes_coords.csv")
    ...
    plt.title(f"Representative CAMELS-US Graph | {huc_str}_{basin_id}", fontsize=14)
    plt.xlabel("Longitude")
    plt.ylabel("Latitude")
    plt.tight_layout()
    out = paths.figures_dir / f"reference_graph_{huc_str}_{basin_id}.png"
